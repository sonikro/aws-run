...
describe('RunCodeInRemoteEnvironment use case', () => {
  const makeSut = () => {
    ...
    const mockedInput: RunCodeInRemoteEnvironmentInput<any> = {
      image: 'ubuntu:latest',
    ...
    const mockedRemoteEnvironment = {
      authenticate: jest
        .fn()
        .mockImplementation(() => Promise.resolve(mockedAuthSession)),
      setup: jest
        .fn()
        .mockImplementation(() => Promise.resolve(mockedEnvironment)),
      tearDown: jest.fn().mockImplementation(() => console.log('Tearing down')),
      execute: jest
        .fn()
        .mockImplementation(() => Promise.resolve(mockedExecutionResult))
    }
  ...
  it('tears down the environment even if execution fails', async () => {
    ...
    mockedRemoteEnvironment.setup = jest
      .fn()
      .mockImplementation(() => Promise.reject(new Error('Setup error')))
    // When
    const sut = new Sut(dependencies)
