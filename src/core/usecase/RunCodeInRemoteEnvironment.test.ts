import { ExecutionResult, ExecutionSettings, RemoteEnvironment } from "../provider/RemoteEnvironment"
import { RunCodeInRemoteEnvironment, RunCodeInRemoteEnvironmentDependencies } from "./RunCodeInRemoteEnvironment"
...
describe("RunCodeInRemoteEnvironment", () => {
    ...
    it("returns the remoteEnvironment execution result", async () => {
        // Given
        const { Sut, dependencies, executionSettings, mockExecutionResult: expectedExecutionResult } = makeSut();
        // When
    ...
    it("throws an error if remote execution fails", async () => {
        ...
        dependencies.remoteEnvironment.execute = jest.fn().mockRejectedValue(expectedError)
        // When
        const runCodeInRemote = new Sut(dependencies)
